...
) -> (Vec<BarArea>, ArrayVec<BarHoleInfo, 2>) {
...
fn map_bar_mapping(
    ...
    if let Err(e) = vm.set_user_memory_region(kvm_memory_region) {
        // SAFETY: host_addr_ptr was returned by a successful mmap call above with the given size.
        let r = unsafe { libc::munmap(host_addr_ptr.cast(), u64_to_usize(size)) };
        if r < 0 {
            error!(
    ...
    if let Err(e) =
        // SAFETY: host_addr_ptr was returned by a successful mmap call with the given size.
        unsafe {
            container.vfio_dma_map(iova, u64_to_usize(size), host_addr_ptr.cast::<u8>())
        }
    {
        let kvm_memory_region = kvm_userspace_memory_region {
            slot,
...
> {
    ...
    if first_vfio_device && let Err(e) = dma_map_guest_memory(container, vm.guest_memory()) {
        for mapping in bar_mappings.iter() {
            unmap_bar_mapping(container, vm.as_ref(), mapping);
        }
        return Err(e);
...
pub fn deinit_vfio_device(
    ...
    _ = device.msix_state.config.vectors.disable();
...
mod tests {
    ...
    /// When MSI-X table and PBA are in the same BAR and close enough that their
    /// page-aligned holes overlap, the gap-punching arithmetic must not underflow.
    /// Here table is at offset 0x0 (32 entries = 512 bytes) and PBA at 0x200 (8 bytes).
    /// Both fall within the same host page, so their page-aligned holes are identical
    /// [0x0, 0x1000). The second gap subtraction `second_gap_offset - offset` underflows.
    #[test]
    fn test_calculate_bar_areas_overlapping_msix_holes() {
        let mut bars = Bars::default();
        // 1 MiB BAR at some address
        bars.set_bar_64(0, 0x4000_0000_0000, 0x10_0000, BarPrefetchable::No);
        ...
        // table_bar=0, table_size=32, table_off=0x0, pba_bar=0, pba_off=0x200
        // Both holes land in the same page [0x0..0x1000)
        let msix_cap = MsixCap::new(0, 32, 0x0, 0, 0x200);

        let (areas, holes) = calculate_bar_areas(&bars, &region_infos, Some(&msix_cap));

        // The overlapping holes merge into a single page-sized gap [0x0..0x1000).
        // So we expect one area covering the rest of the BAR: [0x1000..0x10_0000).
        assert_eq!(areas.len(), 1);
        assert_eq!(areas[0].offset, 0x1000);
        assert_eq!(areas[0].size, 0x10_0000 - 0x1000);

        // Both holes coincide so they should be merged into a single hole
        // carrying both flags.
    ...
    /// Stand-in for `MsixState`. Field names match what the macro expects.
