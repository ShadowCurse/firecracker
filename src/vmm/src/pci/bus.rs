...
#[derive(Default)]
pub struct PciBus {
    /// Devices attached to this bus, indexed by device id.
    /// Slot 0 is the host bridge. A `None` entry means the slot is free.
...
impl PciBus {
    ...
    /// # Errors
    ///
    /// Returns `PciRootError::DuplicateDeviceId` if the slot is already occupied.
    ///
    /// # Panics
    ///
    /// Panics if `device_id >= NUM_DEVICE_IDS` (32).
    pub fn add_device(
        &mut self,
        device_id: u8,
    ...
    /// return the same ID. Callers must hold the bus lock across `next_device_id` and
    /// `add_device` to avoid id collisions when multiple threads attach devices concurrently.
