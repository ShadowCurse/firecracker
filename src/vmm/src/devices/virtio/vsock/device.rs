    ...
    B: VsockBackend + Debug,
{
    ...
    pub fn process_rx(&mut self) -> bool {
        ...
        while let Some(head) = self.queues[RXQ_INDEX].pop_opt() {
            let index = head.index;
            let used_len = match VsockPacket::from_rx_virtq_head_opt(head, mem) {
                Ok(mut pkt) => {
    ...
    pub fn process_tx(&mut self) -> bool {
        ...
        while let Some(head) = self.queues[TXQ_INDEX].pop_opt() {
            let index = head.index;
            let pkt = match VsockPacket::from_tx_virtq_head_opt(head, mem) {
                Ok(pkt) => pkt,
                Err(err) => {
                    error!("vsock: error reading TX packet: {:?}", err);
                    have_used = true;
                    self.queues[TXQ_INDEX]
                        .add_used_opt(index, 0)
                        .unwrap_or_else(|err| {
                            error!("Failed to add available descriptor {}: {}", index, err);
                        });
                    continue;
    ...
    pub fn send_transport_reset_event(&mut self) -> Result<(), DeviceError> {
        ...
        let head = self.queues[EVQ_INDEX].pop_opt().ok_or_else(|| {
        ...
        self.queues[EVQ_INDEX]
            .add_used_opt(head.index, head.len)
            .unwrap_or_else(|err| {
    ...
    B: VsockBackend + Debug + 'static,
{
    ...
    fn activate(&mut self, mem: GuestMemoryMmap) -> Result<(), ActivateError> {
        ...
        self.queues[1].set_pointers(&mem);
        self.queues[2].set_pointers(&mem);
