...
// SAFETY: `File` is Send by construction.
unsafe impl Send for SyncFileEngine {}
// SAFETY: reads and writes use `pread`/`pwrite`, which take the offset per syscall and
// do not modify any shared kernel state on the `File`. That makes it safe to share a
// `SyncFileEngine` across threads via `&self`.
unsafe impl Sync for SyncFileEngine {}
...
impl SyncFileEngine {
    ...
    /// Read `count` bytes from `offset` in the backing file into guest memory at `addr`.
    pub fn read(
        &self,
        offset: u64,
        mem: &GuestMemoryMmap,
    ...
    /// Write `count` bytes from guest memory at `addr` to `offset` in the backing file.
    pub fn write(
        &self,
        offset: u64,
        mem: &GuestMemoryMmap,
