...
pub struct PmemMetricsPerDevice {
    ...
    pub metrics: BTreeMap<String, Arc<PmemDeviceMetrics>>,
}
...
impl PmemMetricsPerDevice {
    ...
    pub fn alloc(drive_id: String) -> Arc<PmemDeviceMetrics> {
        Arc::clone(
            METRICS
                .write()
                .unwrap()
...
pub fn flush_metrics<S: Serializer>(serializer: S) -> Result<S::Ok, S::Error> {
    ...
    let mut pmem_aggregated: PmemDeviceMetrics = PmemDeviceMetrics::default();
    ...
    for (name, metrics) in pmem_metrics.metrics.iter() {
        ...
        let m: &PmemDeviceMetrics = metrics;
...
pub mod tests {
    ...
    fn test_max_pmem_dev_metrics() {
        ...
        // expected.
        let mut metrics: Vec<Arc<PmemDeviceMetrics>> = Vec::new();
        for i in 0..MAX_PMEM_DEVICES {
            let devn: String = format!("drv{}", i);
            metrics.push(PmemMetricsPerDevice::alloc(devn.clone()));
            // update IncMetric
    ...
    fn test_single_pmem_dev_metrics() {
        ...
        // `test_max_pmem_dev_metrics` which also uses the same name.
        let devn = "drv0";

        let test_metrics = PmemMetricsPerDevice::alloc(String::from(devn));
