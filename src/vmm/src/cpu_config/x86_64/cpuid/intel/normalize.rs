...
mod tests {
    ...
    fn make_intel_cpuid(entries: &[kvm_bindings::kvm_cpuid_entry2]) -> IntelCpuid {
        IntelCpuid(kvm_bindings::CpuId::from_entries(entries).unwrap())
    }
    ...
    fn test_update_extended_feature_flags_entry() {
        let mut cpuid = make_intel_cpuid(&[kvm_bindings::kvm_cpuid_entry2 {
            function: 0x7,
            index: 0,
    ...
    fn test_update_extended_topology_v2_entry() {
        let mut cpuid = make_intel_cpuid(&[
