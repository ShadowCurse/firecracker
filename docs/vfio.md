...
- `path_on_host` - sysfs path to the PCI device (e.g.
  `/sys/bus/pci/devices/0000:11:22.3`)
...
2. In the boot source, set `boot_args` so that `root=` names the block device
...
- The guest kernel must include the driver for the passthrough device
  (e.g. `CONFIG_BLK_DEV_NVME=y`) and any filesystem it uses, either built-in or
...
- If the device is hot-plugged after boot it cannot be the root device — the
  ...
  root device.
- The PCI enumeration order in the guest determines the device name. If
  multiple NVMe devices are attached, expect `/dev/nvme0n1`, `/dev/nvme1n1`,
  etc., in BDF order.
