...
def test_api_vfio(uvm_plain):
    ...
    vm.api.vfio.put(id="nvme0", path_on_host="/sys/bus/pci/devices/0003:16:00.0")

    # Overwriting an existing device should be OK
    vm.api.vfio.put(id="nvme0", path_on_host="/sys/bus/pci/devices/0003:16:00.0")

    # Adding a second device should be OK
    vm.api.vfio.put(id="nvme1", path_on_host="/sys/bus/pci/devices/0003:16:00.0")

    # Empty id should fail
    expected_msg = re.escape("Empty device id")
    with pytest.raises(RuntimeError, match=expected_msg):
        vm.api.vfio.put(id="", path_on_host="/sys/bus/pci/devices/0003:16:00.0")
